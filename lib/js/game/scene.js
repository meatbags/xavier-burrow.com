...
class Scene {
  constructor() {
    ...
    this.camera.position.set(2, 1, 3.5);
  ...
  load() {
    ...
    OBJloader.load(`${themePath}/lib/assets/hand.obj`, (obj) => {
      const mat = new THREE.MeshStandardMaterial({metalness: 0.25, roughness: 0.5, wireframe: true});
      this.hand = obj;
      this.hand.children.forEach(child => {
        child.material = mat;
      });
      this.hand.position.set(-100, 0, -100);
  ...
  init(group) {
    // set up the computer
    const texture = new THREE.TextureLoader().load(`${themePath}/lib/assets/texture-blur-invert.jpg`);
    const mat = new THREE.MeshStandardMaterial({metalness: 0.25, roughness: 0.5, map: texture, wireframe: true});
    this.keys = [];
    this.computer = group;
    ...
    const plane = new THREE.Mesh(new THREE.PlaneBufferGeometry(0.8, 0.575), this.noiseShader);
    plane.position.set(-0.25, 0.625, 0.275);
  ...
  setState(index) {
    ...
    if (this.state != index) {
      ...
      this.tweens = [];
      if (this.state == 1) {
        this.tweens.push(new TweenVector(this.camera.position, new THREE.Vector3(2, 1, 3.5),t));
        ...
        this.tweens.push(new TweenVector(this.camera.position, new THREE.Vector3(0, 0.45, 2.5), t));
        this.tweens.push(new TweenVector(this.cameraTarget, new THREE.Vector3(0, 0.45, 0.275), t));
        this.tweens.push(new TweenValue(this, 'screenAlpha', 0.5, t));
