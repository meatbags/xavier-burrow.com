...
import { dot, subVector } from '../maths';
...
class Box {
  ...
  collision(point) {
    return (
      dot(subVector(this.a, point), this.normal.a) >= 0 &&
      dot(subVector(this.b, point), this.normal.b) >= 0 &&
      dot(subVector(this.c, point), this.normal.c) >= 0 &&
      dot(subVector(this.d, point), this.normal.d) >= 0
    );
  }
}
