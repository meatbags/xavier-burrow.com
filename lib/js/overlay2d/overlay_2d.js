...
class Overlay2D {
  ...
  onMouseClick(x, y) {
    ...
    for (var i=0; i<60; ++i) {
      ...
      this.nodes.push(new Node(x + Math.cos(angle) * dist, y + Math.sin(angle) * dist, 50));
    }
  }
  ...
  update(delta) {
    ...
    if (this.activeSection !== null && !this.isMobile && this.nodes.length < this.nodeCount) {
      const n = this.nodeCount - this.nodes.length;
      const rect = this.activeSection.getBoundingClientRect();
      for (var i=0; i<n; ++i) {
        const x = rect.left + Math.random() * rect.width;
        const y = rect.top + Math.random() * rect.height;
        this.nodes.push(new Node(x, y, 120));
      }
    }
