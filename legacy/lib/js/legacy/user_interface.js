...
class UserInterface {
  ...
  parallax(cascade) {
    ...
    if (this.sections.length) {
      ...
      const threshold2 = window.innerHeight * 1.2;
  ...
  openMenu(e) {
    if (!e.classList.contains('active') && !this.menuLock) {
      this.closeCurrentSection();
      this.emptyImagePanes();
      this.menu.querySelectorAll('.active').forEach(el => { el.classList.remove('active'); });
      e.classList.add('active');
      const target = e.dataset.target;
      document.querySelectorAll('.section-wrapper.active').forEach(el => { el.classList.remove('active'); });
      ...
      // fade in/out windows
      if (this.windowFades) {
        this.windowFades.forEach(el => { clearTimeout(el); });
      }
      this.windowFades = [];
      console.log(target);
      if (target === '#section-fun') {
        const windows = document.querySelectorAll('.window');
        for (var i=0, len=windows.length; i<len; ++i) {
          const el = windows[i];
          const fade = setTimeout(() => { el.classList.add('active'); }, 300 * i);
          this.windowFades.push(fade);
        }
      } else {
        document.querySelectorAll('.window').forEach(el => { el.classList.remove('active'); });
      }
    } else {
      this.closeCurrentSection();
      this.emptyImagePanes();
